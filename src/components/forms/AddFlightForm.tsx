...
export default function AddFlightForm({ tripId, segmentId, fromCity, toCity, onClose }: Props) {
  const setTransitToNext = useTripStore((s) => s.setTransitToNext)
  ...
  const valid = departureDate && arrivalDate && cost

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!valid) return
    setTransitToNext(tripId, segmentId, {
      type,
      fromCity,
      toCity,
      departureDate,
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex gap-4">
            <div className="flex flex-1 flex-col gap-2">
              <label style={labelStyle}>Airline / Operator</label>
              <input
                type="text"
